...
add_filter( 'wp_page_menu', 'top_store_add_classes_to_page_menu' );		
     // This theme uses wp_nav_menu() in two locations.
	  function top_store_custom_menu(){
		  ...
		  'top-store-above-menu' => esc_html__( 'Header Above Menu', 'top-x' ),
			'top-store-main-menu'    => esc_html__( 'Main', 'top-x' ),
			'top-store-sticky-menu'  => esc_html__( 'Sticky', 'top-x' ),
			'top-store-footer-menu'  => esc_html__( 'Footer Menu', 'top-x' ),
		) );
	  }
...
function top_store_get_page_title(){ ?>
            ...
            <h2 class="thunk-page-top-title entry-title">
              	<?php printf( __( 'Search Results for: %s', 'top-x' ), '<span>' . esc_html( get_search_query() ) . '</span>' ); ?></h2>
			...
			<?php }else{
			echo'<h1 class="thunk-page-top-title entry-title">'.esc_html__('Blog','top-x').'</h1>'; ?>
			<?php }	 
			 }elseif(is_archive()){
